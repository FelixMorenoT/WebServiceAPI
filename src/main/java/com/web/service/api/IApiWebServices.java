...
@Repository
public interface IApiWebServices extends CrudRepository<UsuariosDTO, Long> {

    @Query("SELECT user.id, user.userName, user.userPassword, user.userMail, user.userStatus FROM UsuariosDTO user where user.userMail = :usermail") 
